...
// CRUD Libros
router.post('/libros/create', verificarToken, crearLibro);
router.get('/libros', obtenerLibros);
router.get('/libros-inhabilitados', obtenerLibrosInhabilitados);
router.get('/libros/:id', obtenerLibroPorId);
router.put('/libros/:id', verificarToken, actualizarLibro);
router.delete('/libros/:id', verificarToken, inhabilitarLibro);

// Reservas
router.post('/libros/:idLibro/reservar', verificarToken, agregarReserva);

router.get('/libros/:idLibro/reservas',
    verificarToken,
    (req, res, next) => {
        if (!req.usuario.permisos.includes('ver_historial')) {
            return res.status(403).json({ mensaje: 'No tienes permiso para ver el historial de reservas' });
...
router.get('/usuarios/:idUsuario/reservas', verificarToken, obtenerReservasPorUsuario);
